...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const updateCourse = (id: number, field: "course" | "semester", value: string) => {
    setCourses(courses.map(c => c.id === id ? { ...c, [field]: value } : c))
  }
  
  const addCourse = () => {
    setCourses([...courses, { id: courses.length + 1, course: "", semester: "" }])
  }
              ...
              <div className="w-full text-center text-sm text-slate-500 space-y-1">
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button variant="link" className="text-slate-500 hover:text-slate-600">
                      Cadastre-se
                    </Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>Cadastro de Novo Usuário</AlertDialogTitle>
                      <AlertDialogDescription>
                        Para realizar seu cadastro no sistema de avaliação institucional,
                       Informe seu e-mail institucional e clique em "Entrar em Contato".
                        ...
                        <div className="space-y-1 pt-4">
                          ...
                          <div className="space-y-1 pt-4">
                          <Label htmlFor="create-password" className="text-sm text-slate-700">
                            Crie uma senha
                          </Label>
                          <div className="relative">
                              <span className="absolute inset-y-0 left-3 flex items-center text-slate-400">
                        ...
                        <div className="space-y-4 pt-4">
                          {courses.map((courseItem, index) => (
                            <div key={courseItem.id} className="space-y-1 relative">
                              {courses.length > 1 && (
                                <button
                                  type="button"
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                  <Label htmlFor={`curso-${courseItem.id}`} className="text-sm text-slate-700">
                                    {index === 0 ? "Informe o(s) curso(s)" : "Curso"}
                                  </Label>
                                  <Select 
                                    value={courseItem.course} 
                                    onValueChange={(value) => updateCourse(courseItem.id, "course", value)}
                                  >
                                    <SelectTrigger className="bg-slate-50 border-slate-200 focus-visible:bg-white">
                                      <SelectValue placeholder="Selecione um curso" />
                                    </SelectTrigger>
                                    <SelectContent>
                                      <SelectItem value="curso1">Análise e Desenvolvimento de Sistemas</SelectItem>
                                      <SelectItem value="curso2">Sistemas de Informação</SelectItem>
                                      <SelectItem value="curso3">Ciência da Computação</SelectItem>
                                    </SelectContent>
                                  </Select>
                                </div>
                                <div className="space-y-1">
                                  <Label htmlFor={`semestre-${courseItem.id}`} className="text-sm text-slate-700">
                                    {index === 0 ? "Semestre atual" : "Semestre"}
                                  </Label>
                                  <Input
                                    ...
                                    onChange={(event) => updateCourse(courseItem.id, "semester", event.target.value)}
                                  />
                                </div>
                              </div>
                            </div>
                          ...
                          <Button
                            ...
                            onClick={addCourse}
                          >
                            <span className="underline">adicionar outro curso</span>
                    ...
                    <AlertDialogFooter>
                      ...
                      <Button type="submit" className="w-auto h-auto bg-emerald-500 hover:bg-emerald-600 text-white font-semibold">Entrar em Contato</Button>
                    </AlertDialogFooter>
                  </AlertDialogContent>
                </AlertDialog>
                <p className="text-xs">
